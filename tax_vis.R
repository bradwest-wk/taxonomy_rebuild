...
edge_file <- "./Reg S-X 5-03.xlsx"
# read in and give proper column names
edges <- read_excel(paste0("./", edge_file), sheet = 1, col_names = T, skip = 0)
...
filename_read <- "Revenues_edges.csv"
...
plot_basic_tree <- function(edgelist, image_name = "tree_basic", folder_name, title = ""){
  ...
  # Create PNG
  png(filename = paste0("./", "figures_", folder_name, "/", image_name, ".png"), width = 5760, height = 5760, res = 100)
  par(cex = 1, cex.main = 5, cex.sub = 4, srt = 270, mar = c(20,4,10,4), font.lab = 2)
  plot(g, layout = layout_as_tree(g), vertex.label.cex = 2, vertex.label = V(g)$name, vertex.color = V(g)$color,
       vertex.frame.color = framecolor, vertex.shape = "circle", vertex.size = 2,
       vertex.label.dist = .1, vertex.label.degree = pi/2, vertex.label.color = "black",
...
usage <- usage_all %>% filter(element_name %in% V(g)$name)
...
# Specify image name
image_name <- "Revenues_current_full_sml_2.png"
...
par(fig = c(0, 1, .01, 1), cex = 1, cex.main = 5, cex.sub = 4, srt = 270, mar = c(20,4,10,4),
    font.lab = 2, new = TRUE)
plot(g, layout = layout_as_tree(g, root = root, rootlevel = c(1, rep(2, 9))), vertex.label.cex = V(g)$label.cex, vertex.label = V(g)$name, vertex.color = V(g)$color,
...
## --------- Whole Enchilada -- With Scale, Color, Title, and Disconnected Nodes ------------ ##
png(filename = paste0("./", "figures_", fig_dir, "/", image_name), width = 5760, height = 5960, res = 100)
plot.new()
# layout(matrix(c(1,2), nrow = 2), heights = c(5760, 200), widths = c(5760, 1000))
par(fig = c(0, 1, .01, 1), cex = 1, cex.main = 5, cex.sub = 4, srt = 270, mar = c(20,4,10,4),
    font.lab = 2, new = TRUE)
...
text(x = .2, y = -1.2, labels = "2016 Element Usage", srt = 0, font = 2, cex = 3, pos = 4)
# Insert heatmap legend
par(fig = c(0, 1, 0, .01), cex = 3, cex.main = 5, cex.sub = 4, srt = 270, par(mar = c(.5,100,0,20)),
    font.lab = 2, new = TRUE)
m <- matrix(1:20, 20, 1)
image(z = m, col = rev(heat.colors(20, alpha = 0.7)), axes = F)
axis(side = 3, at = seq(0, 1, 1/19),
...
edges_full <- bind_rows(edges1, edges2, edges3, edges4, edges5, edges6, edges7, edges8, edges9, edges10)
edges_full_dedup <- edges_full[-which(duplicated(edges_full)), ]

# g1 <- graph_from_edgelist(as.matrix(edges_full), directed = T)
# roots <- which(degree(g1, v = V(g1), mode = "in")==0)
# plot(g1, layout = layout_as_tree(g1, root = roots), vertex.label = NA)
